...
<p><strong>六阶段分析流水线 · LLM 增强层 · 23+ 生产文档 · Token 级成本建模</strong><br/>
<em>从数据集样本或需求文档出发，自动推断 Schema、提取标注规范、估算复刻成本，输出覆盖 6 类角色的生产级文档</em></p>
...
[![Tests](https://img.shields.io/badge/tests-3572_passed-brightgreen.svg)](#开发-development)
[![Coverage](https://img.shields.io/badge/coverage-97%25-brightgreen.svg)](#开发-development)
[![MCP Tools](https://img.shields.io/badge/MCP_Tools-12-purple.svg)](#mcp-服务器-mcp-server)
[![Output](https://img.shields.io/badge/Output-23+_documents-orange.svg)](#输出结构-output-structure)

[摘要](#摘要-abstract) · [问题陈述](#问题陈述-problem-statement) · [形式化框架](#形式化框架-formal-framework) · [架构](#架构-architecture) · [核心创新](#核心创新-key-innovations) · [快速开始](#快速开始-quick-start) · [输出结构](#输出结构-output-structure) · [MCP 服务器](#mcp-服务器-mcp-server) · [生态系统](#生态系统-ecosystem) · [参考文献](#参考文献-references)
...
### 数据集 Schema 推断 / Dataset Schema Inference
...
```mermaid
graph LR
    I["输入<br/>HF 数据集 / PDF / Word"] --> P["解析器<br/>文档提取"]
    P --> A1["Schema<br/>推断"]
    A1 --> A2["评分标准<br/>提取"]
    A2 --> A3["Prompt<br/>提取"]
    A3 --> A4["成本<br/>建模"]
...
### 六阶段分析流水线 / Six-Stage Analysis Pipeline

| 阶段 | 模块 | 产出 |
|:---|:---|:---|
| 1. Schema 推断 | `deep_analyzer.py` | 字段结构、类型、约束、分布 |
| 2. 评分标准提取 | `rubrics_analyzer.py` | 评分标准、标注维度 |
| 3. Prompt 提取 | `prompt_extractor.py` | Prompt 模板、变量结构 |
| 4. 成本建模 | `phased_model.py` · `token_analyzer.py` | 分阶段成本、Token 分析 |
| 5. 人机分配 | `human_machine_split.py` | 人工/机器分配比例 |
| 6. 行业基准对比 | `industry_benchmark.py` | 行业基准对比 |

### 面向角色的输出 / Stakeholder-Oriented Output
...
### 1. LLM 增强层 / LLM Enhancement Layer
...
### 6. Agent 可消费的输出 / Agent-Ready Output
...
```python
from datarecipe import DeepAnalyzer

analyzer = DeepAnalyzer()
result = analyzer.analyze("tencent/CL-bench")
result.save("./projects/CL-bench/")
```
...
| 分析 | **DataRecipe** | 逆向工程 · Schema 推断 · 成本建模 · LLM 增强分析 | 当前项目 |
